...
const KboBoardDetail = () => {
  ...
  return (
    <div className="noticeboard-detail-container" style={{ maxWidth: 600, margin: '40px auto', background: 'white', borderRadius: 10, boxShadow: '0 4px 24px rgba(0,0,0,0.08)', padding: 32 }}>
      ...
      {/* 댓글 영역 끝 */}
      <button onClick={() => navigate(-1)} style={{margin:50, marginTop: 24, padding: '6px 50px', background: '#3498db', color: 'white', border: 'none', borderRadius: 10, fontWeight: 500, cursor: 'pointer' }}>목록으로</button>
      <button onClick={() => navigate(`/PostEditForm2/${post.id}`)} style={{margin:50, marginTop: 24, padding: '6px 50px', background: '#3498db', color: 'white', border: 'none', borderRadius: 10, fontWeight: 500, cursor: 'pointer' }}>수정</button>
    </div>
  );
};
